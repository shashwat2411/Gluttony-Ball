...
// プロトタイプ宣言
//*****************************************************************************
HRESULT InitInput(HINSTANCE hInst, HWND hWnd);//初期化　一回呼び出す
void UninitInput(void);						//終了処理　最後に一回呼び出す
void UpdateInput(void);						//1フレームに一回呼び出す

//---------------------------- keyboard
bool GetKeyboardPress(int nKey);	//押せっぱなしOK
bool GetKeyboardTrigger(int nKey);	//押しっぱなし禁止
bool GetKeyboardRepeat(int nKey);	//最初はトリガー、時間経過でプレスに変化
bool GetKeyboardRelease(int nKey);	//ボタンを離したときにON
